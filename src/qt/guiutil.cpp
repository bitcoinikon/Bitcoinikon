...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitcoinikon"))
        return false;
...
{
    // Convert bitcoinikon:// to bitcoinikon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitcoinikon://"))
    {
        uri.replace(0, 12, "bitcoinikon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoinikon.lnk";
}
...
{
    return GetAutostartDir() / "bitcoinikon.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bitcoinikon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitcoinikon-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bitcoinikon-Qt"));
